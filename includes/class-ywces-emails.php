		...
		public function get_mail_body( $type, $coupon_code, $first_name, $last_name, $user_email, $args = array(), $vendor_id = '' ) {
			...
			switch ( $type ) {
				...
				case 'product_purchasing':
					...
					$replace[] = $args['order_date'];
					$replace[] = $this->render_mailbody_link( wc_get_product( $args['product'] ), 'product' );
					break;
			...
			$mail_body     = str_replace( $find, $replace, $mail_body );
		...
		public function get_coupon_info( $coupon_code ) {
			...
			if ( $coupon_id ) {
				...
				if ( $post ) {
					...
					<h2>
						<?php echo __( 'Coupon code: ', 'yith-woocommerce-coupon-email-system' ) . yit_get_prop( $coupon, 'code' ); ?>
					</h2>
					...
					<?php if ( $expiry_date != '' ) : ?>
						<p>
							<br />
							<b>
								<?php printf( __( 'Expiration date: %s', 'yith-woocommerce-coupon-email-system' ), ucwords( date_i18n( get_option( 'date_format' ), $expiry_date ) ) ); ?>
							</b>
						</p>
					<?php endif; ?>
